...
**What it captures:** For each pair, clean and disturbed versions are shown one at a time, fullscreen, in temporal sequence — never side by side. Each condition displays for 4 seconds, in randomized order per trial (also randomized across pairs), separated by a 2200ms neutral-gray blank so the eye reaches a stable adaptation state for each condition before the next one appears. After both conditions have been shown, a neutral gray judgment screen asks "First / Same / Second," resolved internally to clean_better / disturbed_better / same, alongside two "show again" buttons that let the participant re-view either condition before committing. Re-views are paced the same as the original presentation — blank pause, stimulus, blank pause, back to judgment — rather than an instant toggle, so a re-view can't skip the adaptation reset the whole design depends on. Like an eye exam, this lets a distracted or uncertain trial be redone without discarding the trial's data; the number of re-views is recorded per trial so unusually indecisive pairs can be identified in analysis. 16 comparisons per run. Judgment input is only accepted once the judgment stage is reached — a keypress during a stimulus or the blank is ignored rather than misattributed to the wrong trial.
...
- Re-viewing a condition changes the phenomenon being measured, a little, every time: each additional viewing is itself another adaptation event, so a trial re-viewed several times isn't quite the "fresh first exposure" a zero-replay trial is. The replay count is logged specifically so this can be accounted for (e.g. weighting or filtering high-replay trials) rather than silently treated as equivalent data.
